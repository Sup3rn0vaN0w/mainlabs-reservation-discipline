\section{The reservation discipline: full specification}
...
footprint, a service class, a provenance class (below), and optionally a deadline. Estimates may be wrong; the semantics below are
...
it, and class-serves the rest, which is the surviving shape of Appendix D's first lesson. Second, the pool's projected commitment, the sum over
...
token budget fits within the flow's class quota. Fifth, the flow's provenance class is reservation-eligible (below). A flow failing
...
set with margin, misestimation is charged to the misestimating flow's
own disposition (below), and the forecast window's construction is an
...
its pinned cache, or reclaim its committed budget. The sole override is a halt issued by the management plane (below), which is a safety
...
of Section 2, and the evaluation of Sections 4 through 6 prices both
...
is the departure from precedent flagged in Appendix D's second lesson.
...
trust-keyed admission below adds provenance to the keying. The signal
choice is a design claim the evaluation tests directly: queue depth
reports congestion that has already arrived; the ledger reports
congestion that has been promised; whether the forward signal is worth
anything is answered, against this design's expectation, in Sections 6.5
and 7.
...
within which flow-level contracts are written. Appendix B states the admission test and the ledger invariants at pseudocode level; the
...
them as hard test failures (Section 5).
...
which the stack is least trustworthy about itself. Appendix D's third lesson states the doctrine networks derived from the same observation.
...
objects. Halt revokes a reservation and stops a flow: the one override of the guarantee above, triggerable only by this authority and never by
a capacity condition. Steer forces a degradation action from the ladder above onto a specific flow, a bounded intervention short of
...
6's adversarial results measure exactly this property. The management
...
and Section 8 fixes it as a non-goal: this layer enforces decisions
...
The discipline's final element keys the admission decision above to provenance, the transfer of Appendix D's fourth lesson. Every flow
...
their content, receive capped budgets, and sit lowest in the class-differentiated degradation curves above, shedding first
under pressure.
...
Composed with the management plane above, the enforcement reading is direct: untrusted
