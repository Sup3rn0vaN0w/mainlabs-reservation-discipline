...
under submission to MLSys 2027). Every number in the paper derives from the manifests,
...
| `paper/` | the paper source: one generated LaTeX tree, the MLSys 2027 build target and a no-line-numbers reading variant, verified `references.bib`, the unmodified `mlsys2025.sty`/`mlsys2025.bst` the 2027 CFP directs authors to, and the built PDF. Sections and appendices are GENERATED by `gen_sections.py` from the frozen markdown; never hand-patch them |
...
```
cd paper
python3 gen_sections.py   # regenerate sections and appendices from the markdown
pdflatex main_mlsys && bibtex main_mlsys && pdflatex main_mlsys && pdflatex main_mlsys
```
...
This artifact is DERIVED, never authored in place: it is built by
`build_artifact_repos.py` in the source repository from an explicit
include-list, so nothing outside that list can arrive by accident.
`MANIFEST.sha256` covers every released file and is regenerated on each
build. The paper was compressed from its frozen form for the MLSys page
limit across six gates; the retired TMLR and arXiv build targets are the
record of a superseded submission and are deliberately NOT shipped here,
so that this release contains exactly one paper.
